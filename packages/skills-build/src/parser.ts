...
/**
 * Get category from file path (parent directory name)
 */
...
/**
 * Extract section number from category folder using section map
 */
...
function extractExamples(body: string): CodeExample[] {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (currentLabel && codeBlockContent.length > 0 && line.trim()) {
			...
			if (line.startsWith("#") || line.startsWith("Reference") || line.startsWith("**Source") || line.startsWith("**Verification")) {
				continue;
			}
...
function extractReferences(body: string): string[] {
	...
	for (const line of lines) {
		...
		// Match "**Source Pointer:**" format
		const sourceMatch = line.match(/\*\*Source Pointer:\*\*\s*(.+)/);
		if (sourceMatch) {
			references.push(sourceMatch[1].trim());
			continue;
		}
...
export function parseRuleFile(
	...
	try {
		...
		// Extract section from category folder
		const section = getSectionFromCategory(category, sectionMap);
		if (section === null) {
			errors.push(
				`Could not determine section from category folder: ${category}`,
			);
